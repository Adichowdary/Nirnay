...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!validateInspectionTransition(inspection.status, "CLOSED")) {
    return fail(`Cannot close inspection in ${inspection.status} status`);
  }
  ...
  return ok({ id, status: "CLOSED" }, "Inspection closed");
}
