...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!validateInspectionTransition(inspection.status, "ARRIVED")) {
    return fail(`Cannot mark arrival for inspection in ${inspection.status} status`);
  }
  ...
  await auditLog(supabase, {
    actor_id: user.id,
    action: "INSPECTION_ARRIVED",
    resource: "inspections",
    resource_id: id,
    project_id: inspection.project_id,
  });

  return ok({ id, status: "ARRIVED" }, "Inspector arrived at facility");
}
