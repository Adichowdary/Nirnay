...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!validateInspectionTransition(inspection.status, "SUBMITTED")) {
    return fail(`Cannot submit inspection in ${inspection.status} status`);
  }
  ...
  return ok({ id, status: "SUBMITTED" }, "Inspection submitted for review");
}
