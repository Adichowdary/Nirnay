...
export function MapLayerControl({ layers, onLayersChange }: MapLayerControlProps) {
  ...
  if (collapsed) {
    return (
      <button
        ...
        className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl cursor-pointer transition-all shadow-md bg-slate-900/90 text-white border border-slate-700 backdrop-blur-md hover:bg-slate-800 text-xs font-bold"
      >
        <Layers size={13} className="text-amber-400" />
        <span>Layers</span>
        <span className="hidden sm:inline">&amp; Bhuvan</span>
      </button>
    );
  }
  ...
  return (
    <>
      ...
      />
      <div
        className="rounded-2xl overflow-hidden shadow-2xl bg-slate-900/95 border border-slate-700 text-white backdrop-blur-md z-50 fixed sm:static inset-x-3 top-16 sm:inset-x-auto sm:top-auto w-[calc(100vw-24px)] max-w-xs sm:w-[245px]"
      >
        {/* Header */}
